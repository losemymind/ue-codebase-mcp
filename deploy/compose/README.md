...
Preflight checks the Docker and Compose clients, fully qualified regular-file inputs, reparse points, PEM markers, placeholder/latest images, digest pins and the rendered Compose model. It performs no image pull, container creation, service start or other deployment mutation. A successful preflight is not installation evidence.
