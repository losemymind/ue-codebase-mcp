...
`nginx.conf` is paired with the exact-version, digest-pinned NGINX image supplied through `EDGE_PROXY_IMAGE`. It terminates TLS on container port 8443, disables access logging, limits error output to critical events, bounds headers, bodies, connections, requests and upstream timeouts, forwards only the MCP endpoint, OAuth protected-resource metadata and liveness/readiness, and returns `404` for `/metrics` and every other path. Metadata and health GET routes reject both content-length and transfer-encoded request bodies before proxying.
