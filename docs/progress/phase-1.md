...
- The next confirmed Docker Desktop install exposed a Windows PowerShell 5.1/PowerShell 7 parameter-set incompatibility: `Split-Path -LiteralPath` already selects its only literal-path parent behavior and cannot be combined with `-Parent`. The live database manager and the two existing P1-18 deployment scripts now use the compatible form, tests reject recurrence, and status refreshes process `PATH` before detecting a newly installed Docker CLI. Docker Desktop 4.89.0 was installed, but no database resource or live acceptance result existed at the point of failure.
...
### P1-18 local database live acceptance — verified development environment

On 2026-09-03 the user-approved installer created and verified the isolated local development database. This is actual evidence for the disposable PostgreSQL migration and control-plane runtime harnesses on this workstation only; it is not production control-plane assembly or clean fixed-device deployment evidence.

- Docker Desktop 4.89.0 and Docker CLI 29.7.2 ran the healthy fixed container `ue-codebase-mcp-postgres-test` on `127.0.0.1:55432`. The requested `pgvector/pgvector:0.8.6-pg17` image resolved to `sha256:cf134a767f474095eeba57e0117be8e568e011a63f33fbf252f14c9b760f8e6f`; the live extension reported pgvector `0.8.6`.
- The complete version-9 destructive harness passed: empty upgrade, bootstrap upgrade, negative constraints, rollback through every Phase 1 migration and re-upgrade. The final version-9 schema was then reapplied.
- The real Node.js `pg` pool passed exact migration checksum readiness, its approved named query, a transaction and bounded shutdown using the protected file-backed DSN. The manager recorded status `Verified` at `2026-09-03T01:51:03.4512057+00:00`.
- Live execution exposed and corrected three defects that static/mocked tests had not detected: PowerShell consumed PostgreSQL `-v` as `-Verbose`; migration 8 depended on absent `pgcrypto.gen_random_bytes`; and PostgreSQL rejected two POSIX regular-expression repetition bounds above 255. The adapter now consumes raw arguments, migration 8 uses built-in `gen_random_uuid`, and the 512-character policies use explicit length checks plus unbounded character-class validation.
- Real `pg` also rejected frozen driver query-configuration objects even though the mock driver accepted them. Driver-owned configuration objects are now mutable while the approved SQL policy, caller values and returned boundary remain controlled and immutable.

The user then requested uninstall and selected `E:\Temp\backup`. The manager streamed a 146,299-byte custom-format dump to `ue-codebase-mcp-postgres-20260903T020007Z.dump`, validated its `PGDMP` header and matched SHA-256 `d8a9ac1b9952252e203a9e8568723ad91e9ffa27ff47ed977ff4678ea9985f12` against the protected JSON manifest before removing the fixed container and volume. The first backup attempt safely stopped before `pg_dump` because PowerShell resolved two names for the same Docker executable; `Get-Application` now selects one application deterministically. A stale state file discovered during post-uninstall verification led to literal allowlisted cleanup plus explicit absence checks. Final status was `NotInstalled`, with no managed container or volume and only the protected Docker dependency receipt retained; Docker, Node.js and WSL were not removed.

Production database grants/TLS, retrieval and request-audit assembly, object storage, immutable control-plane image approval and the fixed-device TLS/service rehearsal remain open. The successful local install/verify/backup/uninstall lifecycle does not satisfy those blockers. P1-18 is still in progress and Phase 2 remains frozen.
