...
function config() {
  return validateAgentConfig({
    ...
    claim_wait_ms: 0, idle_delay_ms: 10,
  });
}
...
function manifest(hash = 'a'.repeat(64)) {
  return {
    ...
    generation_id: 'generation-1', revision_set_hash: hash,
    manifest_uri: 'artifact://index-store/generations/generation-1.json', manifest_sha256: 'b'.repeat(64),
  };
}

async function register(coordinator, agentId = 'agent-01') {
  await coordinator.register({
    schema: 'ue-codebase-mcp/agent-register', version: 1, agent_id: agentId,
...
test('expired leases are recovered, old attempts are fenced, and completion is idempotent', async () => {
  ...
  const first = await coordinator.claim({ schema: 'ue-codebase-mcp/job-claim', version: 1, agent_id: 'agent-01', supported_kinds: ['reindex'], wait_ms: 0 }, auth);
  assert.equal(first.lease.attempt, 1);
  clock.advance(1_001);
  assert.equal(coordinator.recoverExpiredLeases(), 1);
  const second = await coordinator.claim({ schema: 'ue-codebase-mcp/job-claim', version: 1, agent_id: 'agent-01', supported_kinds: ['reindex'], wait_ms: 0 }, auth);
