...
function parseLease(value: unknown, path: string): JobLease {
  ...
  exactFields(object, ['job_id', 'agent_id', 'attempt', 'lease_expires_at'], [], path);
  return {
...
export function validateAgentConfig(value: unknown): AgentConfig {
  ...
  exactFields(object, ['schema', 'version', 'agent_id', 'agent_version', 'coordinator_endpoint', 'credential', 'capabilities', 'claim_wait_ms', 'idle_delay_ms'], [], 'agent configuration');
  literal(object.schema, ['ue-codebase-mcp/windows-agent'], 'agent configuration.schema');
  literal(object.version, [AGENT_PROTOCOL_VERSION], 'agent configuration.version');
...
export function validateJobPayload(value: unknown): AgentJobPayload {
  ...
  literal(object.version, [AGENT_PROTOCOL_VERSION], 'job payload.version');
...
export function validateCompletionManifest(value: unknown): CompletionManifest {
  ...
  literal(object.version, [AGENT_PROTOCOL_VERSION], 'completion manifest.version');
  return Object.freeze({
    schema: 'ue-codebase-mcp/reindex-result',
    version: 1,
    generation_id: stringValue(object.generation_id, 'completion manifest.generation_id', AGENT_ID),
    revision_set_hash: stringValue(object.revision_set_hash, 'completion manifest.revision_set_hash', SHA256, 64),
    manifest_uri: stringValue(object.manifest_uri, 'completion manifest.manifest_uri', /^artifact:\/\/[a-z][a-z0-9-]{1,62}\/[A-Za-z0-9._/-]{1,512}$/),
...
export function assertCredential(credential: ShortLivedCredential, now: Date): AgentAuth {
  if (typeof credential.token !== 'string' || credential.token.length < 16 || credential.token.length > 8192) reject('AGENT_CREDENTIAL_INVALID', 'short-lived agent credential is invalid');
